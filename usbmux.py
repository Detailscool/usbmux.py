...
class SafeStreamSocket:
    ...
    def recv(self, size):
        msg = ''
        while len(msg) < size:
            chunk = self.sock.recv(size - len(msg)).decode('utf-8')
            if chunk == '':
                raise MuxError("socket connection broken")
            msg = msg + chunk
        return msg
...
class BinaryProtocol(object):
    ...
    def _pack(self, req, payload):
        if req == self.TYPE_CONNECT:
            return struct.pack("IH", payload['DeviceID'], payload['PortNumber']) + "\x00\x00".encode('utf-8')
    ...
    def sendpacket(self, req, tag, payload={}):
        res = self._pack(req, payload)
        payload = res.encode('utf-8') if type(res) == str else res
        if self.connected:
    ...
    def getpacket(self):
        if self.connected:
            raise MuxError("Mux is connected, cannot issue control packets")
        dlen = self.socket.recv(4).encode('utf-8')
        dlen = struct.unpack("I", dlen)[0]
        body = self.socket.recv(dlen - 4)
        version, resp, tag = struct.unpack("III", body[:0xc].encode('utf-8'))
...
class PlistProtocol(BinaryProtocol):
    ...
    def getpacket(self):
        ...
        res = payload.encode('utf-8') if type(payload) == str else payload
        payload = plistlib.loads(res)
